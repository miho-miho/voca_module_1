...
app.post('/:lang/v/c_detail=:category', (req, res) => {
  ...
  const query = {
    ...
    text: 'SELECT T1.*, T2.* FROM t_usage JOIN t_word ON t_usage.word_id = t_word.id as T1 JOIN (SELECT * FROM t_usage_inst_rel JOIN t_instance ON t_usage_inst_rel.inst_id = t_instance.id ORDER BY t_usage_inst_rel.disp_priority) as T2 ON T2.usage_id=T1.usage_id WHERE T1.word_id = $1 ORDER BY T1.disp_priority',
    values: [targetWordId]
  };

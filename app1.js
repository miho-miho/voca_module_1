...
app.get('/:lang/v/t_search_list=:chuno', (req, res)=> {
  ...
  client.connect();
  const query = "SELECT t_usage.usage_id,t_usage.word_id,chukoumoku_no,basic,midasi FROM t_usage_classified_rel JOIN t_usage ON t_usage_classified_rel.usage_id=t_usage.usage_id JOIN t_word ON t_usage.word_id=t_word.id WHERE t_usage_classified_rel.chukoumoku_no = ?";

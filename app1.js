...
app.get('/:lang/v/catego', (req, res) => {
  ...
  client.connect();
  const query = {
    text: "SELECT t_scene.id AS scene,t_word.basic,t_usage.explanation,t_usage.word_id FROM t_usage_scene_rel JOIN t_usage ON t_usage_scene_rel.usage_id=t_usage.usage_id JOIN t_word ON t_usage.word_id=t_word.id JOIN t_scene ON t_usage_scene_rel.scene_id=t_scene.id"
  };
...
app.get('/:lang/v/table', (req, res) => {
  ...
  client.connect();
  const query = {
    text: "SELECT t_usage.usage_id,t_usage.word_id,rui,chukoumoku_no,chukoumoku,basic,midasi FROM t_usage_classified_rel JOIN t_usage ON t_usage_classified_rel.usage_id=t_usage.usage_id JOIN t_word ON t_usage.word_id=t_word.id WHERE t_usage.selected='1'"
  };
  client.query(query, (err, result) => {
    if (err) throw err;
...
app.post('/:lang/v/c_detail=:category', (req, res) => {
  ...
  const query = {
    ...
    //text: 'SELECT t_usage.explanation,t_instance.* FROM t_usage_inst_rel JOIN t_usage ON t_usage.usage_id=t_usage_inst_rel.usage_id JOIN t_instance ON t_usage_inst_rel.inst_id=t_instance.id WHERE t_usage.usage_id IN (SELECT t_usage.usage_id FROM t_usage JOIN t_word ON t_usage.word_id=t_word.id WHERE t_word.id=$1 ORDER BY t_usage.disp_priority) ORDER BY t_usage_inst_rel.disp_priority',
    text: 'SELECT T.* FROM ( SELECT t_word.id AS word_id, t_usage.usage_id, t_usage.explanation, t_instance.* FROM t_usage_inst_rel JOIN t_instance ON t_usage_inst_rel.inst_id = t_instance.id JOIN t_usage ON t_usage_inst_rel.inst_id = t_usage.usage_id JOIN t_word ON t_usage.word_id = t_word.id) T WHERE T.word_id = $1',
    values: [targetWordId]
  };

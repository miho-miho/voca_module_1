...
app.post('/:lang/v/c_detail=:category', (req, res) => {
  ...
  const query = {
    ...
    text: 'SELECT * FROM   ( SELECT * FROM t_usage_inst_rel JOIN t_instance ON t_usage_inst_rel.inst_id = t_instance.id JOIN t_usage ON t_usage_inst_rel.inst_id = t_usage.usage_id JOIN t_word ON t_usage.word_id = t_word.id WHERE t_word.id = $1)',
    values: [targetWordId]
  };

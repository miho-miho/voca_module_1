...
app.post('/:lang/v/c_detail=:category', (req, res) => {
  ...
  const query = {
    ...
    text: 'SELECT t_usage_inst_rel.inst_id FROM t_usage_inst_rel JOIN t_usage ON t_usage.usage_id=t_usage_inst_rel.usage_id WHERE t_usage.usage_id IN (SELECT t_usage.usage_id FROM t_usage JOIN t_word ON t_usage.word_id=t_word.id WHERE t_word.id=$1)',
    values: [targetWordId]
  };

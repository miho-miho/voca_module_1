...
app.post('/:lang/v/search_detail=:tag', (req, res) => {
  ...
  client.connect();
  const query = {
    text: 'SELECT t_word.basic, t_word.id, t_usage.usage_id, t_usage.explanation, T.targetlanguage, T.trans, T.function, T.pronun, T.module_id, T.explanation as t_ex, T.xml_file_name, T.xpath, T.web_url, T.inst_id as instid, T.token, T.token_index, file_name.t_usage_photo FROM t_usage LEFT OUTER JOIN t_usage_scene_rel ON t_usage.usage_id=t_usage_scene_rel.usage_id JOIN t_word ON t_usage.word_id = t_word.id JOIN (SELECT * FROM t_usage_inst_rel JOIN t_instance ON t_usage_inst_rel.inst_id = t_instance.id) as T ON T.usage_id=t_usage.usage_id JOIN t_usage_photo ON T.usage_id=t_usage_photo.usage_id WHERE t_word.id=any($1) AND t_usage.selected=1 ORDER BY t_usage.disp_priority, T.disp_priority',
